...
import com.example.mouri.common.dto.BookLectureDto;
...
public class CommonController {
	 @Autowired
		...
		public ResponseEntity<BookLectureDto> getAllBooksByLectureId(@PathVariable(value = "lectureId", required = true) Long lectureId) {
	     List<BookDto> bookDtos = bookService.getAllBooksByLectureId(lectureId);
	     BookLectureDto bookLectureDto = new BookLectureDto();
	     bookLectureDto.setLectureId(lectureId);
	     bookLectureDto.setBooks(bookDtos);
	       return ResponseEntity.ok(bookLectureDto);
		}
}
